...
Models:
    Titulacion: Represents an academic degree or program with associated subjects and
        credits.
    Asignatura: Represents an academic subject with attributes for name, credits, and
        semester.
...
Models Managers:
    UsrDaManager: Custom manager for creating users and superusers with additional
        logic.

Each model includes detailed attributes and relationships to represent the academic
structure and user roles within the system.
"""
...
from titulaciones.models import Asignatura, Titulacion
...
class UsrDaManager(BaseUserManager):
    ...
    def create_user(self, preferred_username, email, es_profesor=False, **extra_fields):
        """
        Creates and returns a new user with the specified username, email, and
            additional fields.
...
class UsrDa(AbstractBaseUser, PermissionsMixin):
    ...
    Attributes:
        preferred_username (CharField): The preferred username of the user. Must be
            unique.
        ...
        recuento_visualizaciones (int): The count of views made by the user.
            Defaults to 0.
        es_profesor (bool): A flag indicating whether the user is a professor.
            Defaults to False.
        is_staff (bool): A flag indicating whether the user has staff privileges.
            Defaults to False.
    ...
    titulacion = models.ForeignKey(
        Titulacion, on_delete=models.SET_NULL, blank=True, null=True
    )
...
class Profesor(models.Model):
    ...
    Attributes:
        user (OneToOneField): A one-to-one relationship linking the Profesor to a UsrDa
            instance.
