...
Models:
    Apunte: Represents a study note or document uploaded by a user.
        It includes attributes such as title, file path, creation date, associated
        subject, download count, uploader, associated professors, and a description.

    ejercicios: Represents an exercise with attributes such as statement, optional PDF
        file, creation date, associated subject, associated topic, view count, and
        uploader.
"""

from django.db import models
from titulaciones.models import Asignatura
from usrs.models import Profesor, UsrDa
...
class Apunte(models.Model):
    ...
    Attributes:
        ...
        user (ForeignKey): A foreign key to the UsrDa model, representing the user who
            uploaded the note.
        apoyo_docente (ManyToManyField): A many-to-many relationship with the Profesor
            model, representing the professors who support or are associated with the
            note.
        descripcion (CharField): A textual description of the note. Defaults to an
            empty string.
        visualizaciones (IntegerField): The number of times the note has been viewed.
            Defaults to 0.
...
class Ejercicios(models.Model):
    ...
    Attributes:
        ...
        asignatura (ForeignKey): A foreign key linking the exercise to anAsignatura
            instance. Allows null values and sets the field to NULL if the related
            Asignatura is deleted.
        visualizaciones (IntegerField): The number of times the exercise has been
            viewed. Defaults to 0.
        visible (BooleanField): A flag indicating whether the exercise is visible to
